    ...
    <el-card>
        ...
        <el-row type="flex" justify="center">
            ...
            >
                ...
                <el-form-item label="部门" required prop="departments">
                    <el-select v-model="form.departments":disabled="isAdmin?false:true" placeholder="选择部门">
                        <el-option
                            v-for="i of departs"
                            :key="i.id"
                            :label="i.name"
        ...
        methods: {
            async getData() {
                ...
                if (this.editMode) {
                    ...
                    data.role.departments = data.role.departments.map(i => i.id).shift();
                    this.fillForm(data.role);
                } else {
                    ({data} = await createAdminRole());
                    if (!data.isAdmin) {
                        data.role.departments = data.role.departments.id;
                        this.fillForm(data.role);
                    }

                }
                this.perms = data.permissions;
                this.departs = data.departments;
